      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full border px-2.5 py-1 text-xs ${statusTone(task.status)}`}>{task.status}</span>
          ...
          <p className="text-sm text-zinc-400">Updated {formatClock(task.updatedAt, timezone)} · Started {formatClock(task.startedAt, timezone)}</p>
        </div>
